...
export const getAppointmentsSummary = async (_req: Request, res: Response) => {
  try {
    ...
    const doctorsMap = new Map<string, number>(byDoctorRaw.map(r => [r.doctorId, r._count._all]));
    const doctors = await prisma.doctorProfile.findMany({
      where: { id: { in: Array.from(doctorsMap.keys()) } },
      include: { user: { select: { firstName: true, lastName: true, email: true } } }
    });
    // Aggregate per-status counts for each doctor
    const doctorStatusMap: Record<string, { pending: number; confirmed: number; completed: number; cancelled: number }> = {};
    for (const r of byDoctorStatusRaw) {
      ...
      if (r.status === AppointmentStatus.PENDING) doctorStatusMap[key].pending += r._count._all;
      if (r.status === AppointmentStatus.CONFIRMED) doctorStatusMap[key].confirmed += r._count._all;
      if (r.status === AppointmentStatus.COMPLETED) doctorStatusMap[key].completed += r._count._all;
      if (r.status === AppointmentStatus.CANCELLED) doctorStatusMap[key].cancelled += r._count._all;
    }
    ...
    const patientsMap = new Map<string, number>(byPatientRaw.map(r => [r.patientId, r._count._all]));
    const patients = await prisma.patientProfile.findMany({
      where: { id: { in: Array.from(patientsMap.keys()) } },
      include: { user: { select: { firstName: true, lastName: true, email: true } } }
    });
    // Aggregate per-status counts for each patient
    const patientStatusMap: Record<string, { pending: number; confirmed: number; completed: number; cancelled: number }> = {};
    for (const r of byPatientStatusRaw) {
      ...
      if (r.status === AppointmentStatus.PENDING) patientStatusMap[key].pending += r._count._all;
      if (r.status === AppointmentStatus.CONFIRMED) patientStatusMap[key].confirmed += r._count._all;
      if (r.status === AppointmentStatus.COMPLETED) patientStatusMap[key].completed += r._count._all;
      if (r.status === AppointmentStatus.CANCELLED) patientStatusMap[key].cancelled += r._count._all;
    }
